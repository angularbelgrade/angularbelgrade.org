...
const descriptions: Record<string, string> = {
  2023: `Welcome to Angular Belgrade Day 2023! Join us on November 4 to explore the latest trends and best practices in the Angular ecosystem.`,
};
...
export function getRouteMetaByYear(year: number): RouteMeta {
  ...
  const image = `https://angularbelgrade.org/banners/angular-belgrade-day-${year}.jpg`;
...
export function getRouteMetaBySpeaker(): RouteMeta {
  ...
  return {
    ...
    meta: (route) => {
      ...
      const description = descriptions[year];
      const image = `https://angularbelgrade.org/banners/angular-belgrade-day-${id}.jpg`;
